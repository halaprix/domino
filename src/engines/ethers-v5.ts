...
import { Contract, utils, BigNumber } from "ethers-v5";
...
export interface ResolverEngine {
  ...
  resolveErc20Bulk(params: { entries: { token: string; owner?: string }[] }): Promise<Erc20TokenResolution[]>;
  resolveErc4626(params: { vault: string; owner?: string }): Promise<Erc4626VaultResolution>;
  resolveErc4626Bulk(params: { entries: { vault: string; owner?: string }[] }): Promise<Erc4626VaultResolution[]>;
}

function createEthersV5Executor(
  mc3: Contract,
...
export function createResolver(
  ...
  const abiInterface = iface ?? new utils.Interface([
